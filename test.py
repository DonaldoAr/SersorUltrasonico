...
# def tiempoAlarma(condicion):
    GPIO.output(ALARMA, condicion)
    time.sleep(t)
    
def tiempoAlarma2(condicion):
    GPIO.output(ALARMA, condicion)
        ...
        while True:
            ...
            # == CUANDO LOS USUARIO ENTRA DE MANERA CORRECTA, SE ACTIVA EL SENSOR1 Y SE ENCUENTRA DESACTIVADO EL SENSOR2 ==
                ...
                while(1):
                    ...
                    print("Estado del pin 22 es:", estado)
                    contador += 1
                    time.sleep(0.01)
                    rel1 = GPIO.input(rele1)
                    rel2 = GPIO.input(rele2)
                    ...
                    if( (rel1 == True) and (rel2 == False) ):
                        ...
                        if( int(contador/100) > 30):
                            ...
                            print("Se salio porque alguin entro")
                            break
                        else:
                            # tiempoAlarma(True, 2*contador/100)
                            # Timer(2*contador/100, tiempoAlarma2, (True))
                            for i in range(int(2*contador/100)):
                                try:
                                    tiempoAlarma2(True)
                                    time.sleep(1)
                                    estado = GPIO.input(22)
...
if __name__ == '__main__':
    threading.Thread(target=checkAlarma).start()
    app.run(host='0.0.0.0', debug=True, port=8000)
    #  threading.Thread(target=app.run(host='0.0.0.0', debug=True, port=8000)).start()
